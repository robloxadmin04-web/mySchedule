/* ============================================================
   mySchedule — Friends + Chat module (Supabase)
   ------------------------------------------------------------
   HOW TO USE:
   1. Add to index.html <head>, BEFORE your existing script.js tag:
        <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
   2. Add this file AFTER supabase-js and AFTER script.js:
        <script src="friends.js"></script>
   3. Fill in SUPABASE_URL and SUPABASE_ANON_KEY below
      (Project Settings > API in your Supabase dashboard).
   4. Add the nav button + view container markup shown at the
      bottom of this file (see INDEX_HTML_SNIPPET comment) to index.html.
...
(function () {
  ...
  function wireFriendsEvents() {
    const signUpBtn = $("#friends-signup-btn");
    const signInBtn = $("#friends-signin-btn");
    ...
    if (signUpBtn) signUpBtn.addEventListener("click", async () => {
      ...
      const pass = $("#friends-password").value;
      const username = $("#friends-username").value.trim();
      try {
    ...
    if (signInBtn) signInBtn.addEventListener("click", async () => {
      const email = $("#friends-email").value.trim();
      const pass = $("#friends-password").value;
      try { await signIn(email, pass); } catch (e) { alert(e.message); }
    });
...
/* ============================================================
   ...
   <div class="view" id="view-friends">
     ...
     <div id="friends-auth">
       ...
       <input type="text" id="friends-username" placeholder="Username (for sign up)">
       <input type="email" id="friends-email" placeholder="Email">
       <input type="password" id="friends-password" placeholder="Password">
       <button class="btn btn-primary" id="friends-signup-btn">Sign Up</button>
       <button class="btn btn-outline" id="friends-signin-btn">Sign In</button>
     </div>
   ...
   3. In <head>, ADD BEFORE your existing script.js tag:
   <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>

   4. At the end of <body>, AFTER script.js, add:
   <script src="friends.js"></script>
   ============================================================ */
