...
function showSetupIfNeeded(){
  ...
  // First visit — show landing page first
  showLandingPage();
}
...
function wireEvents(){
  ...
  // About view
  const aboutLanding = $("#about-show-landing");
  if(aboutLanding) aboutLanding.addEventListener("click", ()=>{
    showLandingPage();
  });
  document.querySelectorAll("#view-about .lp-faq-q").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const item = btn.closest(".lp-faq-item");
      const wasOpen = item.classList.contains("open");
      btn.closest(".lp-faq-featured, .lp-faq-list")
        ?.querySelectorAll(".lp-faq-item.open")
...
function initLandingPage(){
  // Get Started buttons
  ["lp-cta-nav","lp-cta-main","lp-cta-bottom"].forEach(id=>{
    const btn = $("#"+id);
    if(btn) btn.onclick = ()=>{
      hideLandingPage();
      if(!state.setupDone){
  ...
  // FAQ toggles
  document.querySelectorAll("#landing-page .lp-faq-q").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const item = btn.closest(".lp-faq-item");
      const wasOpen = item.classList.contains("open");
  ...
  // Demo tabs
  const stage = $("#lp-demo-stage");
  function showDemo(key){
    const d = DEMO_SCREENS[key];
    if(!d || !stage) return;
    stage.innerHTML = `<div class="lp-demo-screen">
      <h3>${d.title}</h3><p>${d.desc}</p>${d.html}
    </div>`;
  }
  document.querySelectorAll(".lp-demo-tab").forEach(tab=>{
    tab.addEventListener("click", ()=>{
      document.querySelectorAll(".lp-demo-tab").forEach(t=>t.classList.remove("active"));
  ...
  document.querySelectorAll("#landing-page .lp-link").forEach(a=>{
